...
F0 Scaffold
  └─ F1 Config + Auth + API (RTU/APS)
       └─ F2 SIP/WebRTC + call-session entity
            ...
            └─ F7 ПБВ полностью (группы, сетка, меню, звонок, layout)
...
## Фаза F7 — ПБВ полностью

**Цель:** полноценная панель быстрого вызова (web): группы, сетка, звонок по клику, контекстное меню, сценарии раскладки/переноса — по мере согласования уточнения из `case.md`.
...
| **LEARN-026-qcp-layout-edit** | Режим редактирования + сохранение раскладки (fast-dial или согласованное упрощение) | TS, API | DnD/позиции сохраняются |
| **LEARN-027-qcp-group-transfer** | Перенос/дублирование между группами (если в scope «полностью») | TS, Pinia | Сценарий как в wiki ПБВ |

**Зависимости:** F2, F3, F6; call card желателен (F4).  
**Референс:** `.reference/src/widgets/quick-call-panel`, wiki `quick-call-panel.md`.  
**Нарезка LEARN-026/027** может сдвинуться после ответа на открытое уточнение в `case.md`.
